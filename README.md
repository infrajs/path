...
0. Есть предопределённые **папки библиотек**, в которых будет осуществляться поиск файла при наличии в начале адреса символа **"-"**. По умолчанию это одна папка ```vendor/infrajs/``` но список папок расширяется автоматически или в ручную в каждом отдельном случае.
